        ...
        <div className="grid lg:grid-cols-2 gap-6">
          ...
          <Card className="bg-white/70 backdrop-blur-md border-white/60 shadow-sm rounded-xl p-5 flex flex-col">
            ...
            <div className="relative aspect-[16/9] bg-slate-100 rounded-lg overflow-hidden border border-slate-200 mb-3 group">
              <img src={selectedHall.images[idx]} alt={selectedHall.name} className="h-full w-full object-cover transition-opacity duration-300" />
              <button onClick={() => setIdx((idx - 1 + selectedHall.images.length) % selectedHall.images.length)} className="absolute left-2 top-1/2 -translate-y-1/2 grid place-items-center h-10 w-10 rounded-full bg-white/90 shadow text-slate-700 hover:bg-white opacity-0 group-hover:opacity-100 transition-opacity"><ChevronLeft className="h-6 w-6" /></button>
              <button onClick={() => setIdx((idx + 1) % selectedHall.images.length)} className="absolute right-2 top-1/2 -translate-y-1/2 grid place-items-center h-10 w-10 rounded-full bg-white/90 shadow text-slate-700 hover:bg-white opacity-0 group-hover:opacity-100 transition-opacity"><ChevronRight className="h-6 w-6" /></button>
            </div>
            
            <div className="flex gap-3 overflow-x-auto pb-1 mt-auto">
              {selectedHall.images.map((src, i) => (
                <button key={i} onClick={() => setIdx(i)} className={`shrink-0 h-16 w-24 rounded-lg overflow-hidden border-2 transition-all ${i === idx ? "border-blue-600" : "border-transparent opacity-70 hover:opacity-100"}`}>
          ...
          <Card className="bg-white/70 backdrop-blur-md border-white/60 shadow-sm rounded-xl p-5 flex flex-col">
            ...
            <div className="flex-1 flex flex-col items-center justify-center pointer-events-auto py-2">
              <Calendar mode="single" selected={date} onSelect={setDate} className="rounded-md border-0 pointer-events-auto bg-transparent p-0 transform scale-[1.05]" />
            </div>
            ...
            <div className="mt-6 pt-4 border-t border-slate-200/60 flex flex-wrap gap-x-4 gap-y-2 justify-center text-[11px] text-slate-600 font-medium">
              <div className="flex items-center gap-1.5"><span className="h-3 w-3 rounded-full bg-green-500"></span>Day (Available)</div>
              <div className="flex items-center gap-1.5"><span className="h-3 w-3 rounded-full bg-purple-500"></span>Morning (Available)</div>
              <div className="flex items-center gap-1.5"><span className="h-3 w-3 rounded-full bg-yellow-400"></span>Evening (Available)</div>
              <div className="flex items-center gap-1.5"><span className="h-3 w-3 rounded-full bg-red-500"></span>Not Available</div>
              <div className="flex items-center gap-1.5"><span className="h-3 w-3 rounded-full bg-blue-500"></span>GCC Blocked</div>
            </div>
          </Card>
          ...
          <Card className="bg-white/70 backdrop-blur-md border-white/60 shadow-sm rounded-xl p-5">
            ...
            <div className="divide-y divide-slate-200/60 text-[13px]">
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><MapPin className="h-4 w-4 text-blue-500" />Zone</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.zone}</div>
              </div>
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><Map className="h-4 w-4 text-blue-500" />Division</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.division}</div>
              </div>
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><Navigation className="h-4 w-4 text-blue-500" />Address</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.address}</div>
              </div>
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><MapPin className="h-4 w-4 text-blue-500" />Landmark</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.landmark}</div>
              </div>
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><Maximize className="h-4 w-4 text-blue-500" />Total Area</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.totalArea}</div>
              </div>
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><Users className="h-4 w-4 text-blue-500" />Seating capacity</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.capacity}</div>
              </div>
              <div className="flex py-2.5">
                <div className="w-[140px] flex items-center gap-2 text-slate-600"><Car className="h-4 w-4 text-blue-500" />Parking capacity</div>
                <div className="flex-1 font-medium text-slate-800">{selectedHall.parkingCapacity}</div>
              </div>
            </div>
          </Card>
          ...
          <div className="flex flex-col gap-4">
            <Card className="bg-white/70 backdrop-blur-md border-white/60 shadow-sm rounded-xl p-5 flex-1">
              ...
              <div className="divide-y divide-slate-200/60 text-[13px]">
                ...
                <div className="flex py-2.5">
                  <div className="flex-1 text-slate-600 font-medium pr-4">Half Day Security Deposit</div>
                ...
                <div className="flex py-2.5">
                  ...
                  <div className="font-medium text-slate-800">Rs {selectedHall.ebChargePerUnit}</div>
                </div>
                <div className="flex py-2.5">
